"""`user_chat_graph` — docs/05-ai-agent-design.md §2.2, docs/11-coding-standard.md §8.1.

Wires the shared QA node pipeline for the User persona only. Imports only from
`tools/user_tools.py` and `graphs/nodes/` — never `tools/operator_tools.py` (Phase 10),
which is the structural isolation boundary `tests/integration/test_persona_isolation.py`
verifies. `operator_chat_graph.py` (Phase 10) reuses every node below and additionally
wires `classify_add_knowledge_intent`/`route_by_intent`/`generate_summary`.

Node order (docs/05-ai-agent-design.md §2.2's diagram): preprocess_input ->
classify_greeting -> classify_out_of_topic -> embed_question -> similarity_search ->
check_similarity_threshold -> condense_history -> generate_answer -> append_sources ->
persist_message -> log_metrics -> END. Every short-circuit tier (greeting/out-of-topic/
below-threshold) routes its `respond_*` node straight to `persist_message`/`log_metrics`
rather than the diagram's literal (abbreviated) "-> END" — `persist_message` must still
record the canned assistant reply for `/api/messages` history, and `log_metrics` must
still log the short-circuit tier for analytics (docs/07-database-design.md §3.7's
`short_circuit_reason` column exists specifically to capture this for every tier, not
just full RAG) — `append_sources` is skipped since there is nothing to cite.
"""
...
_AfterGreeting = Literal["respond_default_greeting", "classify_out_of_topic"]
_AfterOutOfTopic = Literal["respond_out_of_topic", "embed_question"]


def _route_after_greeting(state: ChatState) -> _AfterGreeting:
    return "respond_default_greeting" if state.get("is_greeting") else "classify_out_of_topic"
...
def build_user_chat_graph() -> CompiledStateGraph[ChatState, None, ChatState, ChatState]:
    ...
    builder.add_conditional_edges(
        ...
        ["respond_default_greeting", "classify_out_of_topic"],
    )
    builder.add_conditional_edges(
    ...
    builder.add_edge("condense_history", "generate_answer")
...
user_chat_graph = build_user_chat_graph()
